...
def main():
    ...
    if len(sites_indSamples.keys())<=6 and len(sites_indSamples.keys())>1:
    ...
    elif len(sites_indSamples.keys())==1:
        print('No Venn Diagram with sites from individual samples is generated as the number of samples is one.')
    else:
    ...
    if len(conditions)<=6 and len(conditions)>1:
        VennDiagrams(replicable_sites_dict, args.output+"_Output/Plots/"+output+"_VennDiagram_ReplicableSites_AcrossConditions")
    elif len(conditions)==1:
        print('No Venn Diagram comparing sites across conditions is generated as there is only one condition.') 
    else:
