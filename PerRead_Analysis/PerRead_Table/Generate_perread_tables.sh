...
#Get assignment type: 
cut -f1,5,6,7 $isoforms | uniq > assignment_type.tsv
...
awk -F'\t' -v OFS='\t' '{x=$1;$1="";a[x]=a[x]$0}END{for(x in a)print x,a[x]}' <(cut -f1,4 assignment_type.tsv) | awk -v OFS="," '{first = $1; $1 = ""; print first, $0; }' - | sed 's/,,/\t/g' - > assigning_events.tsv
